...
> **สิ่งที่ต้องมีก่อนในเครื่อง (Prerequisite):** 
> ต้องติดตั้งโปรแกรม **[Docker Desktop](https://www.docker.com/products/docker-desktop/)** เอาไว้และเปิดให้ทำงานอยู่ (ระบบจะใช้ Docker ในการสร้างฐานข้อมูล Oracle ให้เองอัตโนมัติ เพื่อให้เครื่องนั้นสะอาด ไม่ต้องติดตั้ง Oracle ตัวเต็ม)

**ขั้นตอนการใช้งาน (สำหรับ Windows):**
1. นำโฟลเดอร์ `DBMSFinal` วางที่เครื่องคอมพิวเตอร์
2. ดับเบิลคลิกเปิดไฟล์ **`start.bat`** (หากมีแจ้งเตือนให้กด Run Anyway)
3. ระบบจะทำงานและโหลดโปรแกรมและฐานข้อมูลให้อัตโนมัติในหน้าจอดำ

**ขั้นตอนการใช้งาน (สำหรับ macOS / MacBook):**
1. เปิดแอปพลิเคชัน Terminal
2. ใช้คำสั่ง `cd` เข้าไปที่โฟลเดอร์ `DBMSFinal` (เช่น `cd Downloads/DBMSFinal`)
3. พิมพ์คำสั่งรันสคริปต์: `bash start-mac.sh`
4. ระบบจะทำการเรียก Homebrew มาโหลด Node.js และ Oracle ให้เบื้องหลัง

---

### ⚙️ สิ่งที่ระบบ Auto-Setup ทำให้คุณอัตโนมัติ (หลังรันสคริปต์):
- สร้าง Database Oracle ให้อัตโนมัติ: ดึง Image จาก Docker และสตาร์ทเซิร์ฟเวอร์ฐานข้อมูลให้ทันที (ไม่ต้องมี Oracle ในเครื่อง)
- ตรวจสอบและติดตั้ง **Node.js** (หากเครื่องไม่มี)
- ตรวจสอบและติดตั้ง **Oracle Instant Client** แบบอัตโนมัติ (Middleware สำหรับเชื่อมต่อฐานข้อมูล)
- ดาวน์โหลดไลบรารี NPM (`npm install`)
- **รันฐานข้อมูลอัตโนมัติ (`setup-db.js`):** ระบบจะรอจนกว่า Database จาก Docker จะพร้อมทำงาน แล้วจึงอ่านไฟล์จากโฟลเดอร์ `Table/`, `Table/Procedures/` และ `Data/` นำไปสร้างตารางพร้อมใส่ข้อมูลตัวอย่าง (Mock Data) ลง Database ให้อัตโนมัติ
- เปิดเซิร์ฟเวอร์เว็บแอปพลิเคชัน

🌐 **เมื่อเสร็จขบวนการ ให้เปิดเบราว์เซอร์ไปที่:** `http://localhost:3000`
